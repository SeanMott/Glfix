...
static inline int32_t Glfix_Shader_GetUniformLocation(Glfix_Shader_Shader* shader, const char* name)
{
	if (!shader)
	{
...
Glfix_Shader_Shader* Glfix_Shader_CreateVertex(const char* vertexPath, const char* fragmentPath, const char* geometryPath)
{
	...
	//make shader
	Glfix_Shader_Shader* shader = malloc(sizeof(Glfix_Shader_Shader));
